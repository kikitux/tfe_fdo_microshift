...
resource "helm_release" "tfe" {
  ...
  values = [
    templatefile("${path.module}/overrides.yaml", {
      ...
      pg_address          = "${kubernetes_service.postgres.metadata[0].name}.${var.dep_namespace}.svc.cluster.local:${kubernetes_service.postgres.spec[0].port[0].port}"
      fqdn                = local.fqdn
      ...
      s3_endpoint         = "http://${kubernetes_service.minio.metadata[0].name}.${var.dep_namespace}.svc.cluster.local:${kubernetes_service.minio.spec[0].port[0].port}"
      ...
      redis_host          = "${var.tag_prefix}-redis.${var.dep_namespace}.svc.cluster.local"
      redis_port          = "6379"
